...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if (not file.good()) {
      outlog << "get: ifstream failed: " << strerror (errno) << endl;
   ...
   outlog << "sent " << contents.size() << " bytes" << endl;
}
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ...
   // must recv contents (or else will become corrupted).
   ifstream check(header.filename);
   if (check.good()) {
      check.close();
      outlog << "put: server file already exists" << endl;
      header.command = cix_command::NAK;
      header.nbytes = htonl (errno);
      send_packet (client_sock, &header, sizeof header);
      return;
...
void reply_rm (accepted_socket& client_sock, cix_header& header) {
   ifstream check(header.filename);
   if (not check.good()) {
      outlog << "rm: file does not exist" << endl;
      header.command = cix_command::NAK;
      header.nbytes = htonl (errno);
      send_packet (client_sock, &header, sizeof header);
      return;
